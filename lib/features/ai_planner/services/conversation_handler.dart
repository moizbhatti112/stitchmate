...
class ConversationHandler {
  ...
  final List<ChatMessageData> _conversationHistory = [
    ...
    ChatMessageData(
      ...
      content: "You are a helpful assistant powered by Google Gemini. Provide concise and friendly responses."
    ),
  ];

  List<ChatMessageData> get conversationHistory => List.unmodifiable(_conversationHistory);

  // Add a user message to the conversation
  void addUserMessage(String message) {
    _conversationHistory.add(ChatMessageData(role: MessageRole.user, content: message));
  }

  // Add an assistant message to the conversation
  void addAssistantMessage(String message) {
    _conversationHistory.add(ChatMessageData(role: MessageRole.assistant, content: message));
  }
  ...
  List<ChatMessageData> prepareMessagesForRequest({int maxContextMessages = 10}) {
    // Always keep the system message at the beginning
    final systemMessage = _conversationHistory.first;
    
    // Get the most recent messages for context
    final recentMessages = _conversationHistory.length <= maxContextMessages + 1
        ? _conversationHistory.sublist(1) // Skip system message
        : _conversationHistory.sublist(_conversationHistory.length - maxContextMessages);
  ...
  void updateSystemMessage(String systemPrompt) {
    if (_conversationHistory.isNotEmpty && _conversationHistory.first.role == MessageRole.system) {
      _conversationHistory[0] = ChatMessageData(role: MessageRole.system, content: systemPrompt);
    } else {
      // If no system message exists, add one at the beginning
      _conversationHistory.insert(0, ChatMessageData(role: MessageRole.system, content: systemPrompt));
    }
  }
