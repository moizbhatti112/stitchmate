...
class ChatService {
  ...
  String _model = 'gemini-2.0-flash'; // Default model, will be loaded from config
  ...
  Future<bool> updateModel(String model) async {
    ...
    if (model != 'gemini-2.0-flash') {
      debugPrint('Attempted to change model to $model, enforcing gemini-2.0-flash');
  ...
  Future<String> sendMessage(String message) async {
    try {
      ...
      final List<ChatMessageData> messagesList = _conversation.prepareMessagesForRequest();
      
      // Format the conversation for Gemini API
      final List<Map<String, dynamic>> formattedMessages = messagesList.map((msg) {
      ...
      debugPrint('API Key loaded: ${apiKey.isNotEmpty ? "Valid key found" : "No valid key"}');
      // Check if API key is available
      if (apiKey.isEmpty) {
        throw Exception("API key not set. Please set your Google Gemini API key in settings.");
      }
      
      // Force using gemini-2.0-flash model
      final String modelToUse = 'gemini-2.0-flash';
      // Create the Gemini API URL with the API key
      final Uri uri = Uri.parse('$_baseUrl/models/$modelToUse:generateContent?key=$apiKey');
      ...
      if (response.statusCode == 200) {
        ...
        if (responseText.isEmpty) {
          responseText = "I'm ready to assist you. What would you like to know?";
        ...
        final fallbackMessage = "I couldn't process your request. Please try again later.";
        _conversation.addAssistantMessage(fallbackMessage);
        return fallbackMessage;
      }
    } catch (e) {
      debugPrint('Error in ChatService: $e');
      final errorMessage = "Sorry, I'm having trouble connecting right now. Please check your internet connection and try again.";
  ...
  Future<String> getInitialGreeting() async {
    try {
      ...
      // Create the Gemini API URL with the API key
      final Uri uri = Uri.parse('$_baseUrl/models/$modelToUse:generateContent?key=$apiKey');
      debugPrint('Using model for greeting request: $modelToUse');
      
      final Map<String, dynamic> requestBody = {
        'contents': [
          {
            'role': 'user',
            'parts': [{'text': 'Hello, introduce yourself briefly as a Google Gemini assistant.'}]
          }
        ],
        'generationConfig': {
          'temperature': 0.7,
      ...
      debugPrint('Initial greeting response status: ${response.statusCode}');
      debugPrint('Initial greeting response body: ${response.body}');
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData.containsKey('candidates') && 
          ...
          greeting = responseData['candidates'][0]['content']['parts'][0]['text'] ?? '';
        }
        
        if (greeting.isEmpty) {
          greeting = "Hello! I'm your AI assistant powered by Google Gemini. How can I help you today?";
          debugPrint('No content found in greeting, using fallback');
        }
        
        // Clean up any unusual formatting in the greeting
        greeting = cleanResponse(greeting);
        
        debugPrint('Initial greeting (cleaned): $greeting');
        ...
        const fallbackGreeting = "Hello! I'm your AI assistant powered by Google Gemini. How can I help you today?";
  ...
  Future<List<String>> getAvailableModels() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('Failed to get available models: ${response.statusCode} - ${response.body}');
        return [];
      }
    } catch (e) {
  ...
  String cleanResponse(String text) {
    ...
    debugPrint('Cleaning text: ${text.substring(0, min(100, text.length))}${text.length > 100 ? "..." : ""}');
    ...
    text = text.replaceAll(RegExp(r'(thoughts|reasoning):\s*', caseSensitive: false), '');
    ...
    debugPrint('Cleaned text: ${text.substring(0, min(100, text.length))}${text.length > 100 ? "..." : ""}');
