...
class ChatViewModel extends ChangeNotifier {
  ...
  Future<void> loadInitialGreeting() async {
    ...
    try {
      ...
      addBotMessage("Hello! I'm your AI assistant powered by Google Gemini. How can I help you today?");
    } finally {
      setTyping(false);
    }
  }
  ...
  Future<void> sendMessage(ChatMessage message) async {
    ...
    try {
      ...
      String errorMessage = "Sorry, there was an error processing your request.";
      
      if (e.toString().contains('token') || e.toString().contains('api key')) {
        errorMessage = "There seems to be an issue with the API key. Please check your configuration.";
  ...
  void clearChat() {
    ...
    // Optionally reload initial greeting
    loadInitialGreeting();
  }
  ...
  Future<bool> updateApiKey(String apiKey) async {
    try {
      ...
      if (result) {
        addBotMessage("API key updated successfully. You can now use Google Gemini.");
      } else {
        addBotMessage("There was a problem saving your API key. Please try again.");
      }
      return result;
    } catch (e) {
  ...
  Future<bool> updateModel(String model) async {
    try {
      ...
      if (result) {
        ...
        addBotMessage("There was a problem updating the model. Please try again.");
      }
      return result;
    } catch (e) {
  ...
  Future<bool> testApiConnection() async {
    ...
    try {
      ...
      addBotMessage("Diagnostic: API connection failed. Error: ${e.toString()}");
      setTyping(false);
      return false;
    }
  }
  ...
  void runDiagnostics() async {
    ...
    addBotMessage("Google Gemini API Connection: ${apiConnectionOk ? '✅ Good' : '❌ Failed'}");
    ...
    addBotMessage("Diagnostics complete. If you're seeing errors, please check your API key and internet connection.");
  }
