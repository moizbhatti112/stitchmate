...
import 'package:stitchmate/features/home/plane_transport/api_service/autocomplete_prediction.dart';
import 'package:stitchmate/features/home/plane_transport/api_service/location_api_service.dart';
...
class _LocationInputState extends State<LocationInput>
  ...
  Future<void> _refreshLocationStatus() async {
    ...
    if (mounted) {
      ...
      debugPrint(
        "Location service: ${serviceEnabled ? 'enabled' : 'disabled'}, Permission: $permission",
      );
  ...
  Future<bool> _checkLocationPermission({
    bool showDialogOnDisabled = false,
  }) async {
    ...
    final result = await showDialog<bool>(
      ...
      builder:
          (context) => AlertDialog(
            ...
            content: Text(
              'Please enable your location to use this feature. The map and location selection will not work properly without location services.',
            ),
            actions: <Widget>[
              TextButton(
  ...
  Future<void> getCurrentLocationAndSet() async {
    try {
      ...
      if (!isLocationServiceEnabled || !isPermissionGranted) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                "Location services have been disabled. Please enable them to get current location.",
              ),
      ...
      if (mounted) {
        ...
        String formattedAddress = LocationApiService.formatAddress(
          addressDetails,
        );

        if (formattedAddress.isEmpty) {
          formattedAddress =
              "Location at ${curretnloc.currentPosition.latitude.toStringAsFixed(6)}, ${curretnloc.currentPosition.longitude.toStringAsFixed(6)}";
        }
  ...
  Widget build(BuildContext context) {
    ...
    final showLocationWarning =
        !isInitializing && (!isLocationServiceEnabled || !isPermissionGranted);

    // This build method has been completely rewritten to NEVER show the current location button
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildLocationTextField(context),
        const SizedBox(height: 10),
        // Current location button is completely removed - regardless of isDropoff value
        Divider(),
        _buildLocationSuggestions(size),
        if (showLocationWarning) _buildLocationWarningMessage(),
      ],
    );
  }
  ...
  // The method is kept for reference but never used in the build method
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          return Column(
            children: [
              ListTile(
                leading: Icon(Icons.location_on_outlined, color: lightblack),
                title: Text(
                ...
                onTap: () async {
                  ...
                  if (!isLocationServiceEnabled || !isPermissionGranted) {
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            "Location services have been disabled. Please enable them to continue.",
                          ),
                          action: SnackBarAction(
                            label: 'Settings',
                            onPressed: () async {
                  ...
                  LatLng? selectedLatLng =
                      await LocationApiService.getLatLngFromPlaceId(
              ...
              if (index < placePredictions.length - 1) Divider(),
            ],
          );
        },
      ),
